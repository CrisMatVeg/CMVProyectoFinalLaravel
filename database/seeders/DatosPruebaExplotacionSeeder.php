...
/**
 * Seeder seguro para entornos de EXPLOTACIÓN con datos existentes.
 *
 * - NO trunca ninguna tabla.
 * - Usa firstOrCreate / insertOrIgnore en todo → se puede relanzar sin duplicar.
 * - cristian y heraclio ya existen en producción → se crean como cristian2 y heraclio2.
 *
 * Uso:
 *   php artisan db:seed --class=DatosPruebaExplotacionSeeder
 */
class DatosPruebaExplotacionSeeder extends Seeder
{
    private array $tipos;
    ...
    public function run(): void
    {
        ...
        $this->command->info('Datos de prueba cargados en explotación sin borrar datos existentes.');
    }
    ...
    {
        $usuarios = [
            // cristian y heraclio ya existen → se crean con sufijo 2
            ['username' => 'cristian2',  'description' => 'Cristian Mateos',     'email' => 'cristian2@cmv.test'],
            ['username' => 'heraclio2',  'description' => 'Heraclio Borbujo',    'email' => 'heraclio2@cmv.test'],
            // El resto se crean si no existen
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // PROYECTO 1 — Videojuego RPG (owner: heraclio2)
    // ─────────────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // PROYECTO 2 — App Web Corporativa (owner: cristian2)
    // ─────────────────────────────────────────────────────────────────────────
    private function crearProyectoAppWeb(): void
    {
        ...
        $this->acceso($p->id, 'heraclio2', ['Desarrollo']);
        $this->acceso($p->id, 'jorge',     ['Diseño']);
        $this->acceso($p->id, 'lucia',     ['Desarrollo']);
        $this->acceso($p->id, 'marcos',    ['Diseño']);
        $this->acceso($p->id, 'sara',      ['Marketing']);

        $this->tarea($p, 'API REST de gestión de empleados',   'Endpoints CRUD para empleados, departamentos y puestos con autenticación Sanctum.',        'Desarrollo', 'En Proceso',  32, '2026-03-01', '2026-04-20', ['heraclio2', 'lucia']);
        $this->tarea($p, 'Módulo de autenticación con roles',  'Login con 2FA y roles granulares: admin, RRHH y empleado.',                                'Desarrollo', 'Terminada',   16, '2026-02-01', '2026-02-28', ['heraclio2']);
        ...
        $this->tarea($p, 'Sistema de gestión de incidencias',  'Módulo de tickets con estados, prioridades, comentarios y notificaciones.',                 'Desarrollo', 'En Proceso',  20, '2026-03-15', '2026-05-01', ['heraclio2', 'lucia']);
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // PROYECTO 3 — Diseño Editorial (owner: amor)
    // ─────────────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // PROYECTO 4 — Sistema ERP (owner: alberto)
    // ─────────────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // PROYECTO 6 — E-Learning (owner: jorge)
    // ─────────────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // HELPERS — todo con firstOrCreate / insertOrIgnore (sin borrar nada)
    // ─────────────────────────────────────────────────────────────────────────

    private function proyecto(string $nombre, string $desc, string $owner): Proyecto
    {
        return Proyecto::firstOrCreate(
            ['name' => $nombre, 'created_by' => $this->u($owner)],
            ['description' => $desc]
        );
    }
    ...
    private function tarea(
        ...
        $tarea = Tarea::firstOrCreate(
            ['title' => $titulo, 'project_id' => $proyecto->id],
            [
